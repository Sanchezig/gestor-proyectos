        ...
        function openCommentsList(event, projectId, dateKey) {
            ...
            if (comments.length === 0) {
                ...
                <div class="comment-checkbox-wrapper">
                    ...
                    <div style="flex: 1;">
                        <div class="comment-header">${formattedDate} [${currentUser}]${completedBadge}</div>
...
function onDashboardFilterChange(field, value) {
  dashboardFilters[field] = value;
  renderDashboard(); // vuelve a dibujar el dashboard con el filtro nuevo
}
...
function renderDashboard() {
    ...
    // Si no hay año seleccionado, no filtramos nada
    if (!dashboardFilters.endYear) return true;

    // Si el proyecto no tiene fecha fin, lo excluimos cuando hay filtro
    if (!p.endDate) return false;

    // Sacamos el año de la fecha fin
    const year = new Date(p.endDate).getFullYear();
    // Comparamos con el año elegido (guardado como texto)
    return String(year) === dashboardFilters.endYear;
  });

    // Cabecera de la tabla de proyectos
      let html = `
    <div class="dashboard-wrapper">

      <!-- NUEVO: bloque de filtros del dashboard -->
      <div class="dashboard-filters">
        <label class="dashboard-filter-label">
          Año fecha fin:
          <select onchange="onDashboardFilterChange('endYear', this.value)">
            <option value="">Todos</option>
            <option value="2025" ${dashboardFilters.endYear === '2025' ? 'selected' : ''}>2025</option>
            <option value="2026" ${dashboardFilters.endYear === '2026' ? 'selected' : ''}>2026</option>
            <option value="2027" ${dashboardFilters.endYear === '2027' ? 'selected' : ''}>2027</option>
          </select>
        </label>
      </div>
...
`;
      ...
      // Sumatorios para el resumen de ahorro (solo proyectos filtrados)
  let totalVolume = 0;
  let totalFte = 0;
  ...
  filteredProjects.forEach(p => {
    ...
    // Sumamos a los totales (solo de los filtrados)
    totalVolume += volume;
    totalFte += fte;
  ...
  html += `
      ...
      <div class="dashboard-summary-box">
        <div class="summary-label">Total ahorro proyectos filtrados</div>
        ...
        <div class="summary-hint">
          Suma del ahorro estimado en dinero y FTE de los proyectos con fecha fin en el año seleccionado.
        </div>
      </div>
  `;
...
// Cerramos el wrapper general del dashboard
html += `
  </div>
`;
